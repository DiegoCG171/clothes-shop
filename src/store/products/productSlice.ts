...
export const selectFilteredProducts = createSelector(
  [(state: RootState) => state.products.products, (state: RootState) => state.ui.productFilters, (state: RootState) => state.ui.sortBy],
  (products, filters, sortBy) => {
    const filteredProducts = products.filter((product) => {
      const priceNumber = parseFloat(product.price);
      const matchCategory = !filters.categories.length || filters.categories.includes(product.category);
      const matchColor = !filters.color || product.color === filters.color;
      const matchSize = !filters.size || product.size === filters.size;
      const matchPrice = priceNumber >= filters.price[0] && priceNumber <= filters.price[1];
